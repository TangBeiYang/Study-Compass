...
from record_edit import (
    _edit_record_all,
    _edit_record_date,
    _edit_record_category,
    _edit_record_title,
    _edit_record_tags,
    _edit_record_status,
    _edit_record_difficulty,
    _edit_record_blockers,
    _edit_record_note,
)
...
def create_record():
    ...
    category_input = prompt_category(CATEGORY_OPTIONS)
...
def _choose_filtered_records(records, include_back_option: bool):
    ...
    if choice == 2:
        category_chosen = prompt_category(CATEGORY_OPTIONS)
...
def _edit_selected_record(records, record_id: int):
    ...
    while 1:
        menu_edit_action()
        choice = check_input(10)

        if choice == 1:
            records = _edit_record_date(records, index)

        elif choice == 2:
            records = _edit_record_category(records, index)
        
        elif choice == 3:
            records = _edit_record_title(records, index)

        elif choice == 4:
            records = _edit_record_tags(records, index)

        elif choice == 5:
            records = _edit_record_status(records, index)

        elif choice == 6:
            records = _edit_record_difficulty(records, index)

        elif choice == 7:
            records = _edit_record_blockers(records, index)

        elif choice == 8:
            records = _edit_record_note(records, index)

        elif choice == 9:
            records = _edit_record_all(records, index)
...
def _record_actions_menu_loop(records, allowed_ids=None):
    while 1:
        ...
        if choice == 1:
            ...
            print("Confirm deletion for this record:")
            print("1. Yes, delete")
            print("2. No, go back")
            print("Please enter your choice:")
            confirmed = check_input(2)
